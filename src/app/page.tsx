...
export const metadata = {
  title: 'Imóveis de Alto Padrão no Sul de Santa Catarina | Stiven Allan CRECI/RS 60.275',
  description: 'Empreendimentos premium com financiamento direto da construtora, sem burocracia bancária. Sul de SC: Criciúma, Laguna, Rincão, Nova Veneza.',
  alternates: { canonical: SITE_URL },
  openGraph: {
    title: 'Morar bem não deveria depender de um banco. | Stiven Allan',
    description: 'Empreendimentos de alto padrão com financiamento direto da construtora. Sul de Santa Catarina.',
...
async function getEmpreendimentosDB() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, nome, slug, bairro, cidade, uf, status_obra, area_privativa_min, area_privativa_max, imagem_capa_url, construtora')
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 48, marginBottom: 56 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: c.onDarkMuted, lineHeight: 1.75, maxWidth: 260 }}>Especialista em empreendimentos de alto padrão com financiamento direto no Sul de Santa Catarina.</p>
          ...
          <div>
            ...
            {['Criciúma', 'Içara', 'Nova Veneza', 'Forquilhinha', 'Laguna', 'Rincão'].map(cidade => (
              <div key={cidade} style={{ marginBottom: 10 }}>
                <Link href={`/lancamentos/${cidade.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\\s+/g,'-')}-sc`} style={{ fontSize: 13, color: 'rgba(245,241,234,0.55)', textDecoration: 'none' }}>{cidade}/SC</Link>
              </div>
            ))}
          </div>
          <div>
    ...
    <>
      ...
      <section style={{ position: 'relative', minHeight: '100svh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: '#0d0c0b' }}>
        <Image
          ...
          alt="Empreendimento de alto padrão no Sul de Santa Catarina"
        ...
        <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: 1100, margin: '0 auto', padding: 'clamp(110px,14vw,160px) clamp(20px,5vw,56px) clamp(80px,10vw,120px)' }}>
          ...
          <p style={{ fontSize: 'clamp(15px,1.8vw,18px)', color: 'rgba(245,241,234,0.65)', marginBottom: 48, maxWidth: '52ch', lineHeight: 1.7 }}>
            Empreendimentos de alto padrão com financiamento direto da construtora — sem a burocracia do banco.
          </p>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <a href="#empreendimentos"
              style={{ ...ui.btnConvert, boxShadow: '0 8px 28px rgba(255,106,61,0.35)', fontSize: 15, padding: '14px 28px' }}>
      ...
      <section style={{ background: c.paper, padding: 'clamp(56px,7vw,96px) clamp(16px,4vw,56px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 20 }}>
            {REGIOES.map((r) => (
              <a key={r.id} href={`#empreendimentos`}
                style={{ background: c.surface, border: `1px solid ${c.line}`, borderRadius: 6, padding: 'clamp(28px,3vw,40px) clamp(20px,2vw,28px)', textDecoration: 'none', display: 'block', cursor: 'pointer', transition: 'box-shadow .2s' }}
              >
                <div style={{ fontSize: 40, marginBottom: 16, lineHeight: 1 }}>{r.icon}</div>
                <div style={{ fontFamily: font.display, fontWeight: 800, fontSize: 24, letterSpacing: '-0.02em', color: c.ink, marginBottom: 8 }}>{r.label}</div>
                <div style={{ fontSize: 13, color: c.muted, lineHeight: 1.6 }}>{r.cidades.join(' · ')}</div>
            ...
            {portfolio.map((emp: any) => (
              <div key={emp.id} style={{ ...ui.card, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: 'clamp(18px,2vw,24px)', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  ...
                  <div style={{ borderTop: `1px solid ${c.line}`, paddingTop: 16, marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: c.muted }}>Sob consulta</div>
                    <a href={WPP_MSG} target="_blank" rel="noopener noreferrer"
                      style={{ background: c.charcoal, color: c.onDark, fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '9px 16px', borderRadius: 3, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <section style={{ background: c.charcoal, padding: 'clamp(64px,7vw,100px) clamp(16px,4vw,56px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 'clamp(40px,6vw,80px)', alignItems: 'center' }}>
          <div style={{ position: 'relative', height: 'clamp(360px,45vw,560px)', borderRadius: 4, overflow: 'hidden' }}>
            <Image
          ...
          <div>
            ...
            <div style={{ background: 'rgba(245,241,234,0.05)', border: '1px solid rgba(245,241,234,0.1)', borderLeft: '3px solid ' + c.bronze, borderRadius: 4, padding: '20px 24px', marginBottom: 32 }}>
      ...
      {/* 5. COMO FUNCIONA O FINANCIAMENTO DIRETO */}
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 24 }}>
            {[
              ...
              { n: '03', title: 'Sem aprovação bancária', desc: 'Chega de fila, documentos intermitáveis e prazos incertos. A decisão é tomada entre você e a construtora.' },
            ].map((s) => (
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 40, textAlign: 'center', marginBottom: 'clamp(56px,7vw,80px)', paddingBottom: 'clamp(56px,7vw,80px)', borderBottom: '1px solid ' + c.line }}>
            {[
              ...
              { num: '100%', label: 'Foco', desc: 'em alto padrão' },
