...
const jsonLd = {
  ...
  image: IMG.hero,
  url: 'https://stivenallan.vercel.app/empreendimento/fontana/mar-di-nizza-mar-grosso-laguna-sc',
}

const styleBlock = "<style>{" + "`" + `
.mn-hero{position:relative;width:100%;height:100vh;min-height:520px;display:flex;align-items:flex-end}
.mn-hero-img{position:absolute;inset:0;z-index:0}
.mn-hero-overlay{position:absolute;inset:0;z-index:1;background:linear-gradient(to top,rgba(7,13,12,0.72) 0%,rgba(7,13,12,0.18) 60%,rgba(7,13,12,0.04) 100%)}
.mn-hero-content{position:relative;z-index:2;width:100%;max-width:900px;margin:0 auto;padding:0 24px 72px}
.mn-tagline{font-size:clamp(2rem,5vw,3.4rem);font-weight:300;color:#E2F4F2;line-height:1.18;letter-spacing:-.01em;margin:0 0 20px}
.mn-hero-cta{display:inline-flex;align-items:center;gap:10px;background:#1B7A72;color:#fff;text-decoration:none;padding:14px 28px;font-size:.85rem;letter-spacing:.14em;text-transform:uppercase;font-weight:500;border-radius:2px;transition:background .2s}
.mn-hero-cta:hover{background:#104E48}
.mn-section{padding:80px 24px;max-width:1160px;margin:0 auto}
.mn-section-sm{padding:48px 24px;max-width:1160px;margin:0 auto}
.mn-label{font-size:.72rem;letter-spacing:.22em;text-transform:uppercase;font-weight:600;margin:0 0 16px;opacity:.55}
.mn-h2{font-size:clamp(1.8rem,4vw,2.8rem);font-weight:300;letter-spacing:-.02em;margin:0 0 24px;line-height:1.2}
.mn-copy{font-size:1.05rem;line-height:1.75;max-width:680px;opacity:.82}
.mn-stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:2px;background:rgba(15,28,34,0.08);margin-top:56px}
.mn-stat{background:#F8FAFA;padding:28px 20px;text-align:center}
.mn-stat-n{font-size:clamp(1.6rem,3.5vw,2.2rem);font-weight:300;color:#1B7A72;letter-spacing:-.02em;display:block;margin-bottom:4px}
.mn-stat-l{font-size:.72rem;letter-spacing:.18em;text-transform:uppercase;opacity:.55}
.mn-divider{width:100%;height:1px;background:rgba(15,28,34,0.12);margin:0}
.mn-two-col{display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:center}
@media(max-width:768px){.mn-two-col{grid-template-columns:1fr;gap:40px}}
.mn-diff-list{list-style:none;padding:0;margin:0;display:grid;gap:16px}
.mn-diff-item{display:flex;gap:16px;align-items:flex-start}
.mn-diff-num{font-size:.72rem;letter-spacing:.16em;color:#1B7A72;font-weight:600;padding-top:2px;min-width:28px}
.mn-diff-text{font-size:.97rem;line-height:1.5;opacity:.82}
.mn-lazer-card{position:relative;width:100%;aspect-ratio:16/9;overflow:hidden;border-radius:2px}
.mn-gal-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:3px}
@media(max-width:640px){.mn-gal-grid{grid-template-columns:1fr}}
.mn-gcard{border-radius:0}
.mn-onimg{font-family:'Hanken Grotesk',system-ui,sans-serif}
.mn-amenidades{display:flex;flex-wrap:wrap;gap:10px}
.mn-chip{display:inline-flex;align-items:center;padding:8px 16px;border:1px solid rgba(15,28,34,0.16);font-size:.78rem;letter-spacing:.12em;text-transform:uppercase;border-radius:2px;opacity:.8}
.mn-mapa-grid{display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:center}
@media(max-width:768px){.mn-mapa-grid{grid-template-columns:1fr;gap:40px}}
.mn-mapa-img{position:relative;width:100%;aspect-ratio:4/3;overflow:hidden;border-radius:2px}
.mn-cta-band{background:#104E48;color:#E2F4F2;padding:72px 24px;text-align:center}
.mn-cta-band-h{font-size:clamp(1.6rem,3.5vw,2.4rem);font-weight:300;margin:0 0 12px;letter-spacing:-.01em}
.mn-cta-band-sub{opacity:.66;margin:0 0 36px;font-size:1rem}
.mn-cta-btn{display:inline-flex;align-items:center;gap:10px;background:#1B7A72;color:#fff;text-decoration:none;padding:16px 32px;font-size:.85rem;letter-spacing:.14em;text-transform:uppercase;font-weight:500;border-radius:2px;transition:background .2s}
.mn-cta-btn:hover{background:#104E48}
.mn-fab{position:fixed;bottom:28px;right:28px;z-index:999;display:flex;align-items:center;gap:10px;background:#1B7A72;color:#fff;text-decoration:none;padding:14px 22px;border-radius:100px;font-size:.82rem;font-weight:600;letter-spacing:.06em;box-shadow:0 4px 24px rgba(27,122,114,.38);transition:background .2s}
.mn-fab:hover{background:#104E48}
.mn-footer{background:#070D0C;color:rgba(226,244,242,0.66);padding:32px 24px;text-align:center;font-size:.82rem}
` + "`" + "}</style>";
      ...
      <div style={{ fontFamily: t.body, background: t.bg, color: t.ink }}>
        ...
        <header style={{ position:'absolute', top:0, left:0, right:0, zIndex:10, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 32px' }}>
          ...
          <nav style={{ display:'flex', gap:24 }}>
            <a href={CATALOGO} target="_blank" rel="noopener noreferrer" style={{ color:'rgba(226,244,242,0.82)', textDecoration:'none', fontSize:'.8rem', letterSpacing:'.12em', textTransform:'uppercase' }}>Catálogo</a>
