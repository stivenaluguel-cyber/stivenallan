...
const LB_IMGS = [
...GALERIA,
{ src: IMG.piscina, alt: 'Piscina do Lavis Residencial no Centro de Criciúma/SC', label: 'Lazer — Piscina' },
]
...
.lv-gcard { position: relative; overflow: hidden; cursor: zoom-in; }
.lv-gcard img { transition: transform .8s ease; }
.lv-gcard:hover img { transform: scale(1.06); }
.lv-gcard:focus { outline: 2px solid ${t.terra}; outline-offset: 2px; }
...
.lv-lazer-card { position: relative; overflow: hidden; cursor: zoom-in; }
.lv-lazer-card img { transition: transform .8s ease; }
.lv-lazer-card:hover img { transform: scale(1.06); }
.lv-lazer-card:focus { outline: 2px solid ${t.terra}; outline-offset: 2px; }
...
<iframe src={IMG.video} title="Vídeo Lavis Residencial" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', border: 0 }} />
</div>
</section>
...
{GALERIA.map((g, i) => (
<figure key={i} className="lv-gcard" style={{ margin: 0, aspectRatio: '4 / 3', position: 'relative' }} role="button" tabIndex={0} aria-label={`Ampliar imagem: ${g.label}`} onClick={() => openLb(i)} onKeyDown={e => (e.key==='Enter'||e.key===' ') && openLb(i)}>
...
<div className="lv-lazer-card" style={{ position: 'relative', aspectRatio: '4 / 3', overflow: 'hidden' }} role="button" tabIndex={0} aria-label="Ampliar imagem: Lazer — Piscina" onClick={() => openLb(GALERIA.length)} onKeyDown={e => (e.key==='Enter'||e.key===' ') && openLb(GALERIA.length)}>
...
<p style={{ color: t.onDarkMuted, fontSize: 17, lineHeight: 1.6 }}>Rua Timóteo Batista, 53 &mdash; Centro, Criciúma/SC. A poucos passos de tudo o que importa, com a privacidade de um endereço de alto padrão.</p>
