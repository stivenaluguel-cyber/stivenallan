...
// (display + itálico) + Public Sans (corpo/nav/controles) — mesmo sistema de
// PropertyPageTemplate.tsx, EmpreendimentoTemplate.tsx e da home. Esta é a
// única página da Eraldo que não usa o EmpreendimentoTemplate compartilhado,
// por isso os tokens ficam declarados localmente aqui em vez de vir de props.
...
export default function AuraResidencePage() {
  const CSS = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .ar-btn { display: inline-flex; align-items: center; gap: 8px; padding: 14px 32px; font-family: var(--font-public-sans), system-ui, sans-serif; font-size: 15px; font-weight: 600; text-decoration: none; border: 1px solid ${ACCENT}; color: ${ACCENT}; background: transparent; cursor: pointer; transition: all .25s ease; border-radius: 2px; }
    .ar-btn:hover { background: ${ACCENT}; color: #fff; }
    .ar-btn--solid { background: ${ACCENT}; color: #fff; }
    .ar-btn--solid:hover { background: #A43D1B; border-color: #A43D1B; }
    .ar-rule { width: 40px; height: 1px; background: ${ACCENT}; border: none; display: block; }
    .ar-h2 { font-family: var(--font-piazzolla), Georgia, serif; font-weight: 600; line-height: 1.1; }
    .ar-serif { font-family: var(--font-piazzolla), Georgia, serif; font-style: italic; font-weight: 400; }
    .ar-diff-item { display: flex; align-items: flex-start; gap: 16px; padding: 18px 0; border-bottom: 1px solid rgba(26,24,20,0.10); }
    .ar-diff-icon { width: 36px; height: 36px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: rgba(210,78,34,0.08); border-radius: 50%; color: ${ACCENT}; }
    ...
    <>
      ...
      <nav className="ar-nav">
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(18px,4vw,48px)', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            ...
            <a href={WPP} target="_blank" rel="noopener noreferrer" data-wpp="nav" data-wpp-emp={SLUG} data-wpp-nome={NOME} style={{ fontFamily: BODY, fontSize: 15, fontWeight: 600, color: ACCENT_LIGHT, textDecoration: 'none', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>WhatsApp</a>
          </div>
        </div>
      </nav>
      ...
      <section style={{ position: 'relative', height: '100vh', minHeight: 600, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 clamp(24px,6vw,80px)', paddingTop: 68 }}>
          <p className="ar-fade" style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'rgba(245,238,230,0.85)', marginBottom: 24, textShadow: '0 1px 6px rgba(0,0,0,0.4)' }}>
            Eraldo Construções · Centro · Criciúma/SC
          </p>
          <h1 style={{ fontFamily: DISPLAY, fontWeight: 600, lineHeight: 1.15, fontSize: 'clamp(32px,6vw,76px)', color: '#FFFFFF', textShadow: '0 2px 8px rgba(0,0,0,0.5), 0 2px 32px rgba(0,0,0,0.60)', maxWidth: 820 }} className="ar-fade ar-fade-1">
          ...
          <p className="ar-fade ar-fade-3" style={{ marginTop: 20, fontFamily: BODY, fontSize: 13, color: 'rgba(255,255,255,0.65)', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
      ...
      <section id="conceito" style={{ background: '#FAFAF8', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(48px,6vw,96px)', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontFamily: BODY, fontSize: 14, color: '#6B655B', lineHeight: 1.8, marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {[{ Icon: HardHat, label: 'Lançamento' }, { Icon: Building2, label: '17 pavimentos' }, { Icon: House, label: '44 unidades' }, { Icon: CalendarDays, label: 'Entrega abr/2029' }].map(({ Icon, label }) => (
                <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(210,78,34,0.08)', color: ACCENT, borderRadius: 2, padding: '7px 14px', fontFamily: BODY, fontSize: 13 }}><Icon size={14} strokeWidth={1.5} aria-hidden="true" />{label}</span>
              ))}
            </div>
          </div>
          <div style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 4, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: 20, left: 20 }}>
              ...
              <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 18, color: '#F5EEE6' }}>Lançamento · Entrega 04/2029</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="ar-metrics-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 'clamp(32px,4vw,56px)' }}>
            {[{ val: '17', label: 'Pavimentos' }, { val: '44', label: 'Unidades' }, { val: '4', label: 'Coberturas duplex' }, { val: '04/2029', label: 'Entrega prevista' }].map(({ val, label }) => (
              <div key={label} style={{ textAlign: 'center' }}>
                <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 'clamp(38px,5.5vw,58px)', color: 'rgba(245,238,230,0.90)', lineHeight: 1 }}>{val}</p>
          ...
          <div style={{ marginTop: 64, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: 24, borderTop: '1px solid rgba(245,238,230,0.10)', paddingTop: 48 }}>
            {[{ n: '4', label: 'Apartamentos por andar' }, { n: '4', label: 'Pavimentos de garagem' }, { n: '1', label: 'Pavimento de lazer' }, { n: '9', label: 'Espaços de lazer' }].map(({ n, label }) => (
              <div key={label} style={{ textAlign: 'center', padding: '24px 16px' }}>
                <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 'clamp(28px,4vw,48px)', color: ACCENT_LIGHT, lineHeight: 1 }}>{n}</p>
          ...
          <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 32, borderTop: '1px solid rgba(26,24,20,0.10)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
              {FICHA_TECNICA.map(item => (
                <span key={item} style={{ padding: '7px 14px', background: 'rgba(210,78,34,0.07)', borderRadius: 2, fontFamily: BODY, fontSize: 13, color: '#6B655B' }}>{item}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(48px,6vw,96px)', alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {LAZER.map(item => (
                <span key={item} style={{ padding: '7px 14px', background: 'rgba(210,78,34,0.18)', borderRadius: 2, fontFamily: BODY, fontSize: 13, color: ACCENT_LIGHT }}>{item}</span>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="ar-step-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'clamp(32px,4vw,56px)' }}>
            {[
              ...
              <div key={n} style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
                <span style={{ fontFamily: DISPLAY, fontSize: 32, fontWeight: 600, color: 'rgba(210,78,34,0.30)', lineHeight: 1, flexShrink: 0, width: 48 }}>{n}</span>
                <div>
      ...
      <section style={{ background: '#1A1410', padding: 'clamp(80px,10vw,140px) clamp(24px,6vw,80px)', textAlign: 'center' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          ...
          <h2 className="ar-serif" style={{ fontSize: 'clamp(32px,5vw,58px)', color: '#F5EEE6', lineHeight: 1.25, marginBottom: 24 }}>
      ...
      <PropertyFAQ items={FAQ} accent={ACCENT} />
