...
async function getEmpreendimento(slug: string) {
  try {
    ...
    const { data } = await supabase.from('empreendimentos').select('*').eq('slug', slug).single()
    return data
  } catch { return null }
}
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Empreendimentos', item: SITE_URL + '/lancamentos/criciuma-sc' },
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 'clamp(28px,4vw,56px)', alignItems: 'start' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
              {emp.area_privativa_min && <div><div style={{ fontSize: 11, color: c.muted, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Área</div><div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 16, color: c.ink }}>{emp.area_privativa_min === emp.area_privativa_max ? emp.area_privativa_min : emp.area_privativa_min + '–' + emp.area_privativa_max}m²</div></div>}
          ...
          {/* CARD PRECO */}
          <div style={{ background: c.charcoal, color: c.onDark, borderRadius: 4, padding: 'clamp(24px,3vw,32px)', position: 'sticky', top: 64 }}>
            ...
            <p style={{ fontSize: 12, color: c.onDarkMuted, marginTop: 14, textAlign: 'center', lineHeight: 1.5 }}>Atendimento personalizado. Sem compromisso.</p>
          </div>
        </div>
      </section>

      {/* SIMULADOR */}
      <div id="simulador" style={{ background: c.surface, borderTop: `1px solid ${c.line}` }}>
        <Simulador
