  ...
  if (!info) {
        return (
                <main style={{ background: '#121315', minHeight: '100vh', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif', gap: 20 }}>
                          <h1 style={{ fontSize: '2rem', fontWeight: 800 }}>Cidade nao encontrada</h1>h1>
                          <Link href="/" style={{ color: '#c9a24b', textDecoration: 'none', fontSize: 16 }}>Voltar ao inicio</Link>Link>
                </main>main>
              )
  }
                      ...
                      <section style={{ padding: '80px 24px 60px', maxWidth: 1100, margin: '0 auto' }}>
                                <nav style={{ marginBottom: 40 }}>
                                            <ol style={{ display: 'flex', gap: 8, listStyle: 'none', padding: 0, margin: 0, fontSize: 14, color: '#a7adb4' }}>
                                                          <li><Link href="/" style={{ color: '#a7adb4', textDecoration: 'none' }}>Inicio</Link>Link></li>li>
                                                        <li style={{ color: '#c9a24b' }}>&rsaquo;</li>li>
                                                        <li><Link href="/lancamentos/criciuma-sc" style={{ color: '#a7adb4', textDecoration: 'none' }}>Lancamentos</Link>Link></li>li>
                                                        <li style={{ color: '#c9a24b' }}>&rsaquo;</li>li>
                                                        <li style={{ color: '#fff' }}>{info.nome}/{info.uf}</li>li>
                                            </ol>ol>
                                </nav>nav>
                              ...
                              <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.1, marginBottom: 20 }}>
                                        Lancamentos em<br />
                                        <span style={{ color: '#c9a24b' }}>{info.nome}, {info.uf}</span>span>
                              </h1>h1>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 24, marginBottom: 60 }}>
                            {empreendimentos.map((emp, i) => (
                                          <Link key={i} href={emp.slug} style={{ display: 'block', background: '#202327', borderRadius: 12, padding: '28px 24px', border: '1px solid #2e3338', textDecoration: 'none', color: '#fff' }}>
                                                          <p style={{ fontSize: 12, color: '#c9a24b', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>{emp.construtora}</p>p>
                                                          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>{emp.nome}</h2>h2>
                                                          <p style={{ fontSize: 14, color: '#a7adb4', marginBottom: 4 }}>{emp.dorms}</p>p>
                                                          <p style={{ fontSize: 15, fontWeight: 600, color: '#c9a24b', marginBottom: 16 }}>{emp.preco}</p>p>
                                                          <span style={{ fontSize: 13, color: '#c9a24b', fontWeight: 600 }}>Ver detalhes -&gt;</span>span>
                                          </Link>Link>
                                        ))}
                          </div>div>
                        ) : (
                          <div style={{ background: '#202327', borderRadius: 12, padding: '40px 32px', border: '1px solid #2e3338', marginBottom: 60, textAlign: 'center' }}>
                                      <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 12 }}>Em breve lancamentos em {info.nome}</h2>h2>
                                      <p style={{ fontSize: 15, color: '#a7adb4', marginBottom: 24 }}>Cadastre-se para receber novidades em primeira mao.</p>p>
