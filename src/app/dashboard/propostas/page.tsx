        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={e => { if (e.target===e.currentTarget) setModalDetalhe(false) }}>
          <div style={{ background: '#fff', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              ...
              <button onClick={() => setModalDetalhe(false)} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: '#666' }}>×</button>
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={lbl}>Nome do Lead *</label><input style={inp} value={novaForm.lead_nome} onChange={e => setNovaForm(f => ({...f, lead_nome: e.target.value}))} placeholder="Ex: João Silva" /></div>
                <div><label style={lbl}>Telefone/WhatsApp</label><input style={inp} value={novaForm.lead_telefone} onChange={e => setNovaForm(f => ({...f, lead_telefone: e.target.value}))} placeholder="(48) 99999-9999" /></div>
              </div>
              <div><label style={lbl}>Empreendimento</label><input style={inp} value={novaForm.empreendimento_nome} onChange={e => setNovaForm(f => ({...f, empreendimento_nome: e.target.value}))} placeholder="Nome do empreendimento" /></div>
              ...
              <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, marginTop: 4 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div><label style={lbl}>Valor do Imóvel (R$) *</label><input style={inp} value={novaForm.valor_proposto} onChange={e => setNovaForm(f => ({...f, valor_proposto: e.target.value}))} placeholder="R$ 0,00" /></div>
                  <div><label style={lbl}>Entrada/Sinal (R$)</label><input style={inp} value={novaForm.entrada} onChange={e => setNovaForm(f => ({...f, entrada: e.target.value}))} placeholder="R$ 0,00" /></div>
                  <div><label style={lbl}>Qtd. Parcelas Mensais</label><input type="number" style={inp} value={novaForm.parcelas_qtd} onChange={e => setNovaForm(f => ({...f, parcelas_qtd: e.target.value}))} placeholder="Ex: 36" min="0" /></div>
                  <div><label style={lbl}>Valor da Parcela (R$)</label><input style={inp} value={novaForm.parcelas_valor} onChange={e => setNovaForm(f => ({...f, parcelas_valor: e.target.value}))} placeholder="R$ 0,00" /></div>
                  <div><label style={lbl}>Qtd. Reforços</label><input type="number" style={inp} value={novaForm.reforcos} onChange={e => setNovaForm(f => ({...f, reforcos: e.target.value}))} placeholder="Ex: 2" min="0" /></div>
                  <div><label style={lbl}>Valor do Reforço (R$)</label><input style={inp} value={novaForm.reforcos_valor} onChange={e => setNovaForm(f => ({...f, reforcos_valor: e.target.value}))} placeholder="R$ 0,00" /></div>
                  <div style={{ gridColumn: '1 / -1' }}><label style={lbl}>Parcela das Chaves (R$)</label><input style={inp} value={novaForm.chaves_valor} onChange={e => setNovaForm(f => ({...f, chaves_valor: e.target.value}))} placeholder="Saldo na entrega das chaves" /></div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <select style={inp} value={novaForm.status} onChange={e => setNovaForm(f => ({...f, status: e.target.value}))}>
                    {Object.entries(STATUS_COR).map(([k, s]) => <option key={k} value={k}>{s.label}</option>)}
                  </select>
                </div>
              </div>

              <div><label style={lbl}>Observações</label><textarea rows={3} style={{ ...inp, resize: 'vertical' } as React.CSSProperties} value={novaForm.observacoes} onChange={e => setNovaForm(f => ({...f, observacoes: e.target.value}))} placeholder="Condições especiais, notas..." /></div>
            </div>
