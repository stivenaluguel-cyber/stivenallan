...
type Lead = {
  ...
  orcamento_max?: number; origem?: string; created_at?: string; ultimo_contato?: string | null
}
...
// Abaixo desse volume, "score médio"/"análise de pipeline" não diz muita
// coisa de verdade (a IA acabaria comentando sobre 1-2 leads como se fosse
// uma tendência) — melhor deixar claro o motivo do que fingir uma análise.
const MIN_LEADS_PARA_INSIGHTS = 3

const PASSOS_ONBOARDING = [
  { href: '/dashboard/leads', label: 'Cadastrar ou importar seu primeiro lead', icon: '🎯' },
  { href: '/dashboard/empreendimentos', label: 'Importar o estoque de empreendimentos', icon: '🏢' },
  { href: '/dashboard/automacoes', label: 'Configurar a cadência de follow-up automático', icon: '⚙️' },
  { href: '/dashboard/propostas', label: 'Criar sua primeira proposta', icon: '📄' },
  { href: '/dashboard/cron', label: 'Validar se o envio automático (WhatsApp/e-mail) está funcionando', icon: '⏱️' },
]
...
export default function DashboardHome() {
  ...
  const semContato = leads.filter(l => !l.ultimo_contato).length
  ...
  const KPIS = [
    ...
    { l: 'Leads Sem Contato', v: String(semContato), cor: D.amber },
    { l: 'Pipeline Estimado', v: fmt(pipeline), cor: D.ink },
  ]

  const semDadosSuficientes = !loading && total < MIN_LEADS_PARA_INSIGHTS
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: 'clamp(20px,2.5vw,36px) clamp(16px,3vw,32px)' }}>
        ...
        <div style={{ background: D.sidebar, borderRadius: 12, padding: '16px 22px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <div
              title={cub ? `Valor completo: ${formatarCubValor(cub.valor_m2)}/m²` : undefined}
              style={{ fontFamily: "'Bricolage Grotesque',system-ui", fontSize: 'clamp(1.4rem,3vw,2rem)', fontWeight: 800, color: D.bronze, cursor: cub ? 'help' : 'default' }}>
              {cub ? fmt(cub.valor_m2) + '/m²' : (loading ? '—' : 'Sem CUB cadastrado')}
            </div>
            {cub && (
              <div style={{ fontSize: 12, color: D.onDarkMuted, marginTop: 2 }}>
                Competência: {competenciaLabelFromChave(cub.mes_referencia)}
                {statusAtualizacaoCub(cub.mes_referencia) === 'desatualizado' && (
                  <span style={{ color: D.amber, fontWeight: 700 }}> · pode estar desatualizado</span>
                )}
              </div>
            )}
          </div>
        ...
        {!loading && total === 0 ? (
          <div style={{ background: D.surface, border: '1px solid ' + D.line, borderRadius: 12, padding: 24, marginBottom: 28 }}>
            <h2 style={{ fontFamily: "'Bricolage Grotesque',system-ui", fontSize: 18, fontWeight: 800, margin: '0 0 4px', color: D.ink }}>Vamos começar</h2>
            <p style={{ margin: '0 0 18px', fontSize: 14, color: D.muted }}>
              Ainda não há leads cadastrados — em vez de mostrar métricas zeradas, aqui está o que falta pra colocar o sistema pra rodar de verdade.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {PASSOS_ONBOARDING.map((p, i) => (
                <button key={p.href} onClick={() => router.push(p.href)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, background: D.bg, border: '1px solid ' + D.line, borderRadius: 10, padding: '12px 16px', cursor: 'pointer', textAlign: 'left' }}>
                  <span aria-hidden="true" style={{ fontSize: 12, fontWeight: 700, color: D.muted, minWidth: 18 }}>{i + 1}.</span>
                  <span aria-hidden="true" style={{ fontSize: 18 }}>{p.icon}</span>
                  <span style={{ fontSize: 14, fontWeight: 600, color: D.ink }}>{p.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: D.surface, border: '1px solid ' + D.line, borderRadius: 12, padding: 20, marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: insights || insightsErro ? 16 : 0, flexWrap: 'wrap', gap: 12 }}>
            ...
            <button onClick={gerarInsights} disabled={insightsLoading || semDadosSuficientes}
              title={semDadosSuficientes ? `Cadastre pelo menos ${MIN_LEADS_PARA_INSIGHTS} leads para uma análise significativa` : undefined}
              style={{ background: D.bronze, color: '#fff', border: 'none', borderRadius: 8, padding: '10px 18px', fontWeight: 700, fontSize: 13, cursor: (insightsLoading || semDadosSuficientes) ? 'not-allowed' : 'pointer', opacity: (insightsLoading || semDadosSuficientes) ? 0.5 : 1, whiteSpace: 'nowrap' }}>
              {insightsLoading ? 'Gerando...' : (insights ? 'Gerar novamente' : 'Gerar análise')}
            </button>
          </div>
          {semDadosSuficientes && (
            <p style={{ margin: 0, fontSize: 13, color: D.muted, fontStyle: 'italic' }}>
              Dados insuficientes: cadastre pelo menos {MIN_LEADS_PARA_INSIGHTS} leads para gerar uma análise significativa (hoje há {total}).
            </p>
          )}
