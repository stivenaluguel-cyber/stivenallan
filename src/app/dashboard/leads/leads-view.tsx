    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: D.surface, borderRadius: 16, width: '100%', maxWidth: 440, padding: 24, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={onClose} aria-label="Fechar" style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: D.muted, lineHeight: 1 }}>×</button>
        </div>

        <label style={labelStyle} htmlFor="novo-lead-nome">Nome</label>
        <input id="novo-lead-nome" style={inputStyle} value={nome} onChange={(e) => setNome(e.target.value)} placeholder='Nome do lead' />

        <label style={labelStyle} htmlFor="novo-lead-whatsapp">WhatsApp *</label>
        <input id="novo-lead-whatsapp" style={inputStyle} value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder='(48) 99999-9999' />

        <label style={labelStyle} htmlFor="novo-lead-email">E-mail</label>
        <input id="novo-lead-email" style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder='email@exemplo.com' />

        <label style={labelStyle} htmlFor="novo-lead-origem">Origem</label>
        <select id="novo-lead-origem" style={inputStyle} value={origem} onChange={(e) => setOrigem(e.target.value)}>
          {ORIGENS.map((o) => (<option key={o} value={o}>{o}</option>))}
        </select>

        <label style={labelStyle} htmlFor="novo-lead-orcamento">Orçamento máximo (R$)</label>
        <input id="novo-lead-orcamento" style={inputStyle} value={orcamento} onChange={(e) => setOrcamento(e.target.value)} placeholder='Ex: 350000' />

        <label style={labelStyle} htmlFor="novo-lead-estagio">Estágio inicial</label>
        <select id="novo-lead-estagio" style={inputStyle} value={estagio} onChange={(e) => setEstagio(e.target.value)}>
          {ESTAGIOS.map((s) => (<option key={s.key} value={s.key}>{s.label}</option>))}
        </select>

        {erro && <p role="alert" style={{ color: D.red, fontSize: 13, marginTop: 14 }}>{erro}</p>}
...
const [savingNotes, setSavingNotes] = useState(false)
  ...
  // Abre um lead (usado tanto pelo clique no card quanto pela restauração via
  // URL) e sincroniza ?leadId= — sem isso, dar F5 ou usar voltar/avançar do
  // navegador sempre fechava o modal e perdia o lead selecionado.
  const abrirLead = useCallback((l: Lead) => {
    setSelectedLead(l)
    setNotes(l.anotacoes ?? '')
    setEditForm({ nome: l.nome ?? '', whatsapp: l.whatsapp ?? '', email: l.email ?? '', origem: l.origem ?? '', orcamento_max: l.orcamento_max != null ? String(l.orcamento_max) : '' })
    setEditing(false)
    setConfirmDelete(false)
    setEventos([])
    fetch('/api/admin/leads/' + l.id + '/eventos').then(r => r.json()).then(d => setEventos(d.eventos ?? [])).catch(() => setEventos([]))
    const params = new URLSearchParams(searchParams.toString())
    params.set('leadId', l.id)
  ...
  // Restaura o lead aberto a partir de ?leadId= (F5, link direto, ou voltar
  // pelo histórico do navegador) assim que a lista carregar.
  useEffect(() => {
    ...
    <div style={{ minHeight: '100vh', background: D.bg, padding: '24px 20px' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <label htmlFor="busca-leads" style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap' }}>Buscar por nome ou WhatsApp</label>
          <input id="busca-leads" value={busca} onChange={(e) => setBusca(e.target.value)} placeholder='Buscar nome ou WhatsApp...' style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid ' + D.line, background: '#fff', color: D.ink, fontSize: 14, outline: 'none', flex: '1 1 220px', minWidth: 0 }} />
      ...
      {erro && <p role="alert" style={{ color: D.red, fontSize: 14, marginBottom: 16 }}>{erro}</p>}
            ...
            return (
              ...
              >
                ...
                {doEstagio.length === 0 ? (
                  ...
                  doEstagio.map((lead) => (
                    <LeadCard key={lead.id} lead={lead} onDragStart={setDragId} onToggleAtencao={toggleAtencao} onSelect={abrirLead} />
                  ))
                )}
              </div>
            )
...
{selectedLead && (
<div
onClick={(e) => { if (e.target === e.currentTarget) fecharLead() }}
...
{confirmDelete && (<div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '10px', padding: '12px', marginBottom: '16px' }}><p style={{ margin: '0 0 10px', fontSize: '14px', color: '#991B1B', fontWeight: 600 }}>Excluir este lead permanentemente? Essa ação não pode ser desfeita.</p><div style={{ display: 'flex', gap: '8px' }}><button onClick={async () => { await fetch('/api/admin/leads/'+selectedLead.id, { method: 'DELETE' }); setLeads(prev => prev.filter(l => l.id !== selectedLead.id)); fecharLead() }} style={{ flex: 1, background: '#DC2626', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>Sim, excluir</button><button onClick={() => setConfirmDelete(false)} style={{ flex: 1, background: '#fff', color: '#374151', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Cancelar</button></div></div>)}
            {editing ? (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>{(['nome','whatsapp','email'] as const).map(key => (<div key={key}><label htmlFor={`edit-lead-${key}`} style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#52525b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', marginBottom: '4px' }}>{key.charAt(0).toUpperCase()+key.slice(1)}</label><input id={`edit-lead-${key}`} type={key==='email'?'email':key==='whatsapp'?'tel':'text'} value={editForm[key]} onChange={e => setEditForm(prev => ({ ...prev, [key]: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e4e4e7', borderRadius: '10px', padding: '10px 12px', fontSize: '14px', color: '#18181b', backgroundColor: '#fff', outline: 'none', boxSizing: 'border-box' as const, fontFamily: 'inherit' }} /></div>))}<div><label htmlFor="edit-lead-origem" style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#52525b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', marginBottom: '4px' }}>Origem</label><select id="edit-lead-origem" value={editForm.origem} onChange={e => setEditForm(prev=>({...prev,origem:e.target.value}))} style={{ width: '100%', border: '1.5px solid #e4e4e7', borderRadius: '10px', padding: '10px 12px', fontSize: '14px', color: '#18181b', backgroundColor: '#fff', outline: 'none', boxSizing: 'border-box' as const, fontFamily: 'inherit' }}>{['Site','Instagram','Indicacao','Portal','Anuncio','Evento','Whatsapp','Outro'].map(o=><option key={o} value={o}>{o}</option>)}</select></div><div><label htmlFor="edit-lead-orcamento-max" style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#52525b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', marginBottom: '4px' }}>Orçamento máx (R$)</label><input id="edit-lead-orcamento-max" type="number" value={editForm.orcamento_max} onChange={e=>setEditForm(prev=>({...prev,orcamento_max:e.target.value}))} style={{ width: '100%', border: '1.5px solid #e4e4e7', borderRadius: '10px', padding: '10px 12px', fontSize: '14px', color: '#18181b', backgroundColor: '#fff', outline: 'none', boxSizing: 'border-box' as const, fontFamily: 'inherit' }} /></div><div style={{ display: 'flex', gap: '10px' }}><button onClick={async () => { const res = await fetch('/api/admin/leads/'+selectedLead.id, { method: 'PATCH', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ nome: editForm.nome, whatsapp: editForm.whatsapp, email: editForm.email||null, origem: editForm.origem, orcamento_max: editForm.orcamento_max ? Number(editForm.orcamento_max) : null }) }); if (res.ok) { const updated = { ...selectedLead, nome: editForm.nome, whatsapp: editForm.whatsapp, email: editForm.email||null, origem: editForm.origem, orcamento_max: editForm.orcamento_max ? Number(editForm.orcamento_max) : null }; setLeads(prev => prev.map(l => l.id === selectedLead.id ? updated : l)); setSelectedLead(updated); setEditing(false) } }} style={{ flex: 1, background: '#D24E22', color: '#fff', border: 'none', borderRadius: '10px', padding: '12px', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>Salvar alterações</button><button onClick={() => setEditing(false)} style={{ flex: 1, background: '#f3f4f6', color: '#374151', border: 'none', borderRadius: '10px', padding: '12px', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>Cancelar</button></div></div>) : (<table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
...
<label htmlFor="lead-anotacoes" style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '6px' }}>
Anotações
</label>
<textarea
id="lead-anotacoes"
