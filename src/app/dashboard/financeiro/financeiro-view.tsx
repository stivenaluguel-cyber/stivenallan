...
// Mesmo tipo que Home e CRM leem de GET /api/admin/cub — fonte única de
// verdade (src/lib/dashboard/cub-fonte-unica.ts). Antes esta tela tinha seu
// PRÓPRIO histórico isolado em localStorage (nunca sincronizado com
// Supabase), o que causava o valor/competência aqui divergir do que Home e
// CRM mostravam.
type Cub = { valor_m2: number; mes_referencia: string; variacao_mensal?: number | null; updated_at?: string | null }
...
export default function FinanceiroPage() {
  ...
  const [cubHistorico, setCubHistorico] = useState<Cub[]>([])
  ...
  const carregarCub = useCallback(async () => {
    try {
      ...
      setCub(data.vigente ?? null)
      setCubHistorico((data.data ?? []) as Cub[])
    } catch {
      // silencioso — o card mostra "Sem CUB cadastrado" e o botão Atualizar segue disponível
    }
  }, [])
  ...
  // Mesma fonte/persistência que o widget do CRM usa (GET /api/cub → scraper
  // SINDUSCON, POST /api/admin/cub → grava em configuracoes_cub) — antes esta
  // tela salvava só num localStorage próprio, nunca sincronizado com o banco
  // que Home/CRM liam, daí o valor/competência aqui divergir dos outros dois.
  async function atualizarCub() {
    setCubSalvando(true); setCubStatus('')
    try {
      const res = await fetch('/api/cub', { cache: 'no-store' })
      if (!res.ok) throw new Error('Falha ao consultar o SINDUSCON')
      const data = await res.json()
      const mesReferencia = (data.competencia ?? new Date().toISOString().slice(0, 7)) + '-01'
      await fetch('/api/admin/cub', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mes_referencia: mesReferencia, valor_m2: Number(data.valor_m2), variacao_mensal: null }),
      })
      await carregarCub()
      const label = competenciaLabelFromChave(mesReferencia)
      setCubStatus(data.online
        ? 'Atualizado pelo SINDUSCON (' + label + ')'
        : 'SINDUSCON indisponível — usando último valor conhecido (' + label + ')')
    } catch {
      setCubStatus('Não foi possível atualizar agora. Tente novamente em instantes.')
    } finally {
    ...
    <div style={{ padding: '1.5rem', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <label htmlFor="perc-comissao" style={{ fontSize: '0.8rem', fontWeight: 600, color: '#374151' }}>% Comissão:</label>
          <input id="perc-comissao" type="number" min={1} max={6} step={0.5} value={percComissao} onChange={e => setPercComissao(Number(e.target.value))} style={{ width: '70px', padding: '0.4rem 0.5rem', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '0.875rem', textAlign: 'center' }} />
        </div>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.5rem', borderTop: '3px solid #8b5cf6' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>CUB/SC Vigente · SINDUSCON-SC</div>
            {cub ? (
              <>
                <div title={`Valor completo: ${formatarCubValor(cub.valor_m2)}/m²`} style={{ fontSize: '1.6rem', fontWeight: 800, color: '#8b5cf6', cursor: 'help' }}>
                  {formatarCubValor(cub.valor_m2)}<span style={{ fontSize: 14, color: '#9ca3af', fontWeight: 400 }}>/m²</span>
                </div>
                <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>
                  Competência: <strong>{competenciaLabelFromChave(cub.mes_referencia)}</strong>
                  {statusAtualizacaoCub(cub.mes_referencia) === 'desatualizado' && (
                    <strong style={{ color: '#f59e0b' }}> · pode estar desatualizado</strong>
                  )}
                </div>
                {cub.updated_at && <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>Atualizado em {fmtDateTime(cub.updated_at)}</div>}
              </>
            ) : (
        ...
        {cubStatus && (
          <p role="status" style={{ margin: '6px 0 0', fontSize: 13, color: '#16a34a', fontStyle: 'italic' }}>{cubStatus}</p>
        )}
          ...
          <div style={{ marginTop: 16, borderTop: '1px solid #f3f4f6', paddingTop: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {cubHistorico.slice(0, 6).map(c => (
                <div key={c.mes_referencia} style={{ background: '#f5f3ff', borderRadius: 6, padding: '4px 10px', fontSize: 12 }}>
                  <span style={{ color: '#8b5cf6', fontWeight: 700 }}>{formatarCubValor(c.valor_m2)}</span>
                  <span style={{ color: '#9ca3af', marginLeft: 4 }}>· {competenciaLabelFromChave(c.mes_referencia)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <caption style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap' }}>Histórico de vendas e comissões</caption>
              <thead>
        ...
        <div onClick={() => setModalVenda(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, padding: 28, width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              ...
              <button onClick={() => { setModalVenda(false); setModalEditar(false); setEditandoId(null) }} aria-label="Fechar" style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#6b7280' }}>×</button>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label htmlFor="venda-cliente" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Cliente / Lead *</label>
                <input
                  id="venda-cliente"
              ...
              <div>
                <label htmlFor="venda-empreendimento" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Empreendimento</label>
                <input
                  id="venda-empreendimento"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label htmlFor="venda-valor" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Valor da Venda (R$) *</label>
                  <input
                    id="venda-valor"
                ...
                <div>
                  <label htmlFor="venda-entrada" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Entrada (R$)</label>
                  <input
                    id="venda-entrada"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label htmlFor="venda-parcelas-qtd" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Parcelas (qtd)</label>
                  <input
                    id="venda-parcelas-qtd"
                ...
                <div>
                  <label htmlFor="venda-parcelas-valor" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Valor Parcela (R$)</label>
                  <input
                    id="venda-parcelas-valor"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label htmlFor="venda-recebido" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#16a34a', marginBottom: 4, textTransform: 'uppercase' }}>✅ Recebido (R$)</label>
                  <input id="venda-recebido" type="text" value={vendaRecebido} onChange={e => setVendaRecebido(e.target.value)} placeholder="Valor já recebido" style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #d1d5db', borderRadius: 8, fontSize: 14, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label htmlFor="venda-a-receber" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#ea580c', marginBottom: 4, textTransform: 'uppercase' }}>⏳ A Receber (R$)</label>
                  <input id="venda-a-receber" type="text" value={vendaAReceber} onChange={e => setVendaAReceber(e.target.value)} placeholder="Valor pendente" style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #d1d5db', borderRadius: 8, fontSize: 14, boxSizing: 'border-box' }} />
                </div>
              </div>
                            <div>
                <label htmlFor="venda-data" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 4, textTransform: 'uppercase' }}>Data da Venda</label>
                <input
                  id="venda-data"
