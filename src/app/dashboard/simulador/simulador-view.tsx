...
import {
  simular, planos, tabelaCorbetta, tabelaSplit, tabelaGiassi, construtoras,
  TAXA_PADRAO_JS, CUB_MENSAL_REF, gerarMemoriaCalculo,
} from '@/data/financiamento'
...
export default function SimuladorPage() {
  ...
  const [corbEntradaTocada, setCorbEntradaTocada] = useState(false)
  ...
  // ── Estado Ficha da simulação (data-base / validade) ─────────────────────
  const [hoje] = useState(() => new Date())
  const [validadeDias, setValidadeDias] = useState(15)
  const [copiadoMemoria, setCopiadoMemoria] = useState(false)
  const validoAte = useMemo(
    () => new Date(hoje.getTime() + validadeDias * 24 * 60 * 60 * 1000),
    [hoje, validadeDias]
  )
  ...
  const sim = useMemo(() => {
    ...
    return simular(slug, valorImovel, opcoes, correcaoB, hoje)
  }, [slug, valorImovel, opcoes, correcaoB, hoje])
  ...
  // BUG (corrigido): a Tabela de Prazos Corbetta tinha um campo de entrada
  // próprio que começava vazio e nunca era sincronizado com a entrada já
  // resolvida na simulação de cima (sim.parcelaA.entrada) — o corretor
  // configurava a entrada em cima, descia até esta tabela, esquecia de
  // digitar de novo e a tabela financiava o valor CHEIO do imóvel.
  // Sincroniza por padrão, mas para de sincronizar assim que o usuário edita
  // manualmente este campo (flag corbEntradaTocada) — mantém a flexibilidade
  // de simular um cenário de entrada diferente só nesta tabela.
  useEffect(() => {
    if (corbEntradaTocada || !sim) return
    setCorbEntradaRaw(fmtBRL(sim.parcelaA.entrada))
  }, [sim, corbEntradaTocada])
  ...
  // Soma pura do que simular() já retorna — entrada + mensais + reforços +
  // saldo devedor pós-chaves (Parcela B). O trecho pós-chaves é o saldo A
  // FINANCIAR na Parcela B, não o total pago com juros (que depende do
  // sistema escolhido — SPC-JS, Price ou SAC, cada um com valor diferente
  // ao longo do prazo — ver tabela da Parcela B acima). Não fabrica taxa
  // nenhuma nova.
  const totalGeral = useMemo(() => {
    if (!sim) return null
    const totalMensais = sim.parcelaA.valorMensal * sim.parcelaA.qtdMensais
    const totalReforcos = sim.parcelaA.valorReforco * sim.parcelaA.qtdReforcos
    const saldoPosChaves = sim.parcelaB?.saldoDevedor ?? 0
    return {
      entrada: sim.parcelaA.entrada,
      totalMensais,
      totalReforcos,
      saldoPosChaves,
      total: sim.parcelaA.entrada + totalMensais + totalReforcos + saldoPosChaves,
    }
  }, [sim])

  // ── Memória de cálculo (cronograma mês a mês da Parcela A) ───────────────
  const memoriaCalculo = useMemo(() => {
    if (!sim) return []
    const saldoA = sim.valorImovel - sim.parcelaA.entrada
    return gerarMemoriaCalculo(
      saldoA,
      sim.parcelaA.qtdMensais,
      sim.parcelaA.valorMensal,
      sim.parcelaA.qtdReforcos,
      sim.parcelaA.valorReforco
    )
  }, [sim])

  // Mês a mês quando o prazo é curto; agrupado ano a ano quando for muita linha.
  const memoriaAnual = useMemo(() => {
    if (memoriaCalculo.length <= 36) return null
    const porAno = new Map<number, { ano: number; totalMensais: number; totalReforcos: number; saldoFinal: number }>()
    for (const linha of memoriaCalculo) {
      const ano = Math.ceil(linha.mes / 12)
      const atual = porAno.get(ano) ?? { ano, totalMensais: 0, totalReforcos: 0, saldoFinal: 0 }
      atual.totalMensais += linha.parcelaMensal
      atual.totalReforcos += linha.valorReforco
      atual.saldoFinal = linha.saldoDevedorEstimado
  ...
  // ── Copiar memória de cálculo (versão pragmática de "versionar a proposta") ─
  // Não grava no banco nem integra com /dashboard/propostas (mudança de schema
  // maior, fora de escopo aqui) — só monta um texto com as condições completas
  // da simulação e copia pra área de transferência, pro corretor colar onde
  // quiser (proposta formal, WhatsApp, etc.), preservando a versão exata que
  // foi mostrada ao cliente.
  async function copiarMemoriaCalculo() {
    if (!imovelSelecionado || !sim) return
    const { parcelaA, parcelaB } = sim
    const linhas: string[] = [
      '📐 Memória de Cálculo — ' + imovelSelecionado.nome,
      'Empreendimento: ' + imovelSelecionado.nome + ' — ' + imovelSelecionado.cidade + '/' + imovelSelecionado.uf,
      'Valor do imóvel: ' + fmtBRL(valorImovel),
      'Data-base: ' + fmtData(hoje),
      'Validade da simulação: ' + validadeDias + ' dias (válida até ' + fmtData(validoAte) + ')',
      '',
      'Parcela A — durante a obra',
      '• Entrada: ' + fmtBRL(parcelaA.entrada) + ' (' + Math.round((parcelaA.entrada / valorImovel) * 100) + '%)',
      '• Taxa: ' + (TAXA_PADRAO_JS * 100).toFixed(2) + '% a.m.',
    ]
    if (parcelaA.qtdMensais > 0) linhas.push('• ' + parcelaA.qtdMensais + 'x mensais de ' + fmtBRL(parcelaA.valorMensal))
    if (parcelaA.qtdReforcos > 0) linhas.push('• ' + parcelaA.qtdReforcos + ' reforço(s) no total do contrato de ' + fmtBRL(parcelaA.valorReforco) + ' (1 a cada 12 meses)')
    if (parcelaB) {
      linhas.push('')
      linhas.push('Parcela B — pós-chaves (' + parcelaB.correcaoLabel + ')')
      linhas.push('• Saldo a financiar: ' + fmtBRL(parcelaB.saldoDevedor))
      linhas.push('• SPC-JS: ' + parcelaB.meses + 'x de ' + fmtBRL(parcelaB.spcMensal ?? parcelaB.parcelaMensal))
    }
    if (totalGeral) {
      linhas.push('')
      linhas.push('Total estimado do contrato: ' + fmtBRL(totalGeral.total))
    }
    linhas.push('')
    linhas.push('Valores estimados, sujeitos à tabela vigente.')
    await navigator.clipboard.writeText(linhas.join('\n'))
    setCopiadoMemoria(true)
    setTimeout(() => setCopiadoMemoria(false), 2000)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: D.bg, fontFamily: 'system-ui, sans-serif', padding: '28px 20px 64px' }}>
      {/* Exportação simples via impressão do navegador — sem dependência nova
          (nada de jsPDF): esconde controles de edição/botões de ação no
          @media print, mantém visível o resumo, a ficha da simulação e a
          memória de cálculo (cronograma). */}
      ...
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          ...
          <button className="no-print" onClick={() => window.print()} style={{
            padding: '10px 16px', borderRadius: 10, border: '1.5px solid ' + D.line,
            background: '#fff', color: D.ink, fontWeight: 700, fontSize: 13, cursor: 'pointer',
          }}>
            🖨️ Exportar / Imprimir
          </button>
        </div>
        ...
        {/* ── Ficha da simulação ───────────────────────────────────────── */}
        {sim && imovelSelecionado && (
          <div style={{ ...card, borderTop: '3px solid #111827' }}>
            <p style={sectionTitle}>Ficha da simulação</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 16 }}>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Empreendimento</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>{imovelSelecionado.nome}</div>
                <div style={{ fontSize: 12, color: D.muted }}>{imovelSelecionado.cidade}/{imovelSelecionado.uf}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Valor do imóvel</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>{fmtBRL(valorImovel)}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Simulação gerada em</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>{fmtData(hoje)}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Taxa (Parcela A)</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>{(TAXA_PADRAO_JS * 100).toFixed(2)}% a.m.</div>
                <div style={{ fontSize: 11, color: D.muted }}>SPC-JS, juros simples</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Validade da simulação</div>
                <div className="no-print" style={{ marginBottom: 4 }}>
                  <input type="number" min={1} max={365} value={validadeDias}
                    ...
                    style={{ width: 70, padding: '6px 8px', borderRadius: 8, border: '1.5px solid ' + D.line, fontSize: 13 }} />
                  <span style={{ fontSize: 12, color: D.muted, marginLeft: 6 }}>dias</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>Válida até {fmtData(validoAte)}</div>
              </div>
              {correcaoB === 'cub' && sim.parcelaB && (
                <div>
                  <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>CUB/SC usado na Parcela B</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>{(CUB_MENSAL_REF * 100).toFixed(2)}% a.m.</div>
                  <div style={{ fontSize: 11, color: D.amber }}>Estimativa fixa — não é o CUB/SC real vigente.</div>
                </div>
              )}
            </div>
          </div>
        ...
        {sim && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              ...
              {sim.parcelaA.qtdReforcos > 0 && (
                <div style={{ background: D.bg, borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Reforços</div>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#8b5cf6' }}>{fmtBRL(sim.parcelaA.valorReforco)}</div>
                      {/* qtdReforcos é o TOTAL de reforços do plano inteiro (não uma cadência
                          recorrente anual) — a cadência real (1 reforço a cada 12 meses) vem
                          de fatorReforco() em financiamento.ts, exposta aqui explicitamente
                          em vez de inventada. */}
                      <div style={{ fontSize: 12, color: D.muted, marginTop: 2 }}>{sim.parcelaA.qtdReforcos} reforço(s) no total do contrato — 1 a cada 12 meses</div>
                    </div>
            ...
            <div style={{ ...card, marginBottom: 0, borderTop: '3px solid #22c55e', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <button className="no-print" onClick={() => copiarProposta()} style={{
          ...
          <div style={{ ...card, borderTop: '3px solid #3b82f6' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
              ...
              <div>
                ...
                {correcaoB === 'cub' && (
                  <div style={{ fontSize: 11, color: D.amber, marginTop: 4 }}>Estimativa fixa — não é o CUB/SC real vigente.</div>
                )}
              </div>
            </div>
        ...
        {sim && totalGeral && (
          <div style={{ ...card, borderTop: '3px solid #111827' }}>
            <p style={sectionTitle}>Σ Total estimado do contrato</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 14, marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Entrada</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: D.ink }}>{fmtBRL(totalGeral.entrada)}</div>
              </div>
              {sim.parcelaA.qtdMensais > 0 && (
                <div>
                  <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Mensais ({sim.parcelaA.qtdMensais}x)</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: D.ink }}>{fmtBRL(totalGeral.totalMensais)}</div>
                </div>
              )}
              {sim.parcelaA.qtdReforcos > 0 && (
                <div>
                  <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Reforços ({sim.parcelaA.qtdReforcos}x)</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: D.ink }}>{fmtBRL(totalGeral.totalReforcos)}</div>
                </div>
              )}
              {sim.parcelaB && (
                <div>
                  <div style={{ fontSize: 11, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Saldo pós-chaves (Parcela B)</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: D.ink }}>{fmtBRL(totalGeral.saldoPosChaves)}</div>
                </div>
              )}
            </div>
            <div style={{ background: D.bg, borderRadius: 10, padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              <div>
                <div style={{ fontSize: 11, color: D.muted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total ao longo do contrato</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: D.ink }}>{fmtBRL(totalGeral.total)}</div>
              </div>
              <button className="no-print" onClick={copiarMemoriaCalculo} style={{
                padding: '10px 16px', borderRadius: 10, border: 'none',
                background: copiadoMemoria ? '#16a34a' : D.bronze, color: '#fff',
                fontWeight: 700, fontSize: 13, cursor: 'pointer',
              }}>
                {copiadoMemoria ? '✅ Copiado!' : '📐 Copiar memória de cálculo'}
              </button>
            </div>
            {sim.parcelaB && (
              <p style={{ margin: '12px 0 0', fontSize: 11, color: D.muted, lineHeight: 1.5 }}>
                * O saldo pós-chaves (Parcela B) é o valor a financiar — o total efetivamente pago nessa etapa
                depende do sistema de amortização escolhido (SPC-JS, Price ou SAC), cada um com valores diferentes
                ao longo do prazo. Veja a tabela da Parcela B acima para comparar.
              </p>
            )}
          </div>
        )}

        {/* ── Memória de cálculo — cronograma detalhado da Parcela A ───── */}
        {sim && sim.parcelaA.qtdMensais > 0 && (
          <div style={card}>
            <button className="no-print" onClick={() => setShowCronograma(v => !v)} style={{
              width: '100%', background: 'none', border: 'none', cursor: 'pointer',
              display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 0,
            }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: D.ink }}>📅 Memória de cálculo — cronograma da Parcela A</span>
              <span style={{ fontSize: 18, color: D.muted }}>{showCronograma ? '▲ Ocultar' : '▼ Ver'}</span>
            </button>
            ...
            {showCronograma && (
              <div style={{ marginTop: 16, overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 480 }}>
                  <thead>
                    <tr style={{ background: D.bg }}>
                      <th style={{ padding: '9px 12px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: D.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                        {memoriaAnual ? 'Ano' : 'Parcela nº (mês)'}
                      </th>
                      <th style={{ padding: '9px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: D.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                        {memoriaAnual ? 'Total mensais no ano' : 'Valor da parcela'}
                      </th>
                      <th style={{ padding: '9px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: D.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reforço</th>
                      <th style={{ padding: '9px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: D.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Saldo devedor estimado</th>
                    </tr>
                  </thead>
                  <tbody>
                    {memoriaAnual ? (
                      memoriaAnual.map((linha, idx) => (
                        <tr key={linha.ano} style={{ background: idx % 2 === 0 ? '#fff' : D.bg, borderTop: '1px solid ' + D.line }}>
                          <td style={{ padding: '9px 12px', fontWeight: 700, color: D.ink }}>Ano {linha.ano}</td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: D.ink }}>{fmtBRL(linha.totalMensais)}</td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: linha.totalReforcos > 0 ? '#8b5cf6' : D.muted }}>
                            {linha.totalReforcos > 0 ? fmtBRL(linha.totalReforcos) : '—'}
                          </td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: D.muted }}>{fmtBRL(linha.saldoFinal)}</td>
                        </tr>
                      ))
                    ) : (
                      memoriaCalculo.map((linha, idx) => (
                        <tr key={linha.mes} style={{ background: linha.isReforco ? '#FFF3EC' : (idx % 2 === 0 ? '#fff' : D.bg), borderTop: '1px solid ' + D.line }}>
                          <td style={{ padding: '9px 12px', fontWeight: 700, color: D.ink }}>{linha.mes}</td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: D.ink }}>{fmtBRL(linha.parcelaMensal)}</td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: linha.isReforco ? '#8b5cf6' : D.muted }}>
                            {linha.isReforco ? fmtBRL(linha.valorReforco) : '—'}
                          </td>
                          <td style={{ padding: '9px 12px', textAlign: 'right', color: D.muted }}>{fmtBRL(linha.saldoDevedorEstimado)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
                ...
                <p style={{ margin: '12px 0 0', fontSize: 11, color: D.muted, lineHeight: 1.5 }}>
                  * Saldo devedor nominal estimado (soma simples do que falta pagar) — não descontado a valor
                  presente. Derivado diretamente dos mesmos valores de mensal/reforço já calculados acima (SPC-JS).
                </p>
              </div>
            )}
          </div>
        ...
        <div style={{ ...card, borderTop: '3px solid ' + D.bronze }}>
          ...
          {/* Controles Corbetta */}
          <div className="no-print" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
            <div>
              <label style={lbl}>Entrada / Ato (R$)</label>
              <input style={inp} type="text" inputMode="numeric"
                value={corbEntradaRaw}
                onChange={e => { setCorbEntradaRaw(maskBRL(e.target.value)); setCorbEntradaTocada(true) }}
                placeholder="R$ 0,00" />
              {/* BUG corrigido: este campo sincroniza por padrão com a entrada já
                  resolvida na simulação de cima (sim.parcelaA.entrada) — antes
                  começava sempre vazio e a tabela financiava o valor cheio do
                  imóvel se o corretor esquecesse de repetir a entrada aqui. */}
              {!corbEntradaTocada && sim && (
                <div style={{ fontSize: 11, color: D.muted, marginTop: 4 }}>
                  (sincronizado com a entrada acima — edite para usar outro valor)
                </div>
              )}
            </div>
            <div>
          ...
          {isCorbetta && (
            <div className="no-print" style={{ background: '#FFF8F5', border: '1px solid #F9C4B1', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
          ...
          {corbLinhaAtiva && corbTabela.length > 0 && (() => {
            ...
            return (
              <div style={{ marginTop: 16, padding: '14px 16px', background: '#FFF3EC', borderRadius: 12, border: '1px solid #F9C4B1', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                ...
                <button className="no-print" onClick={() => copiarProposta({ prazo: corbLinhaAtiva.prazo, modo: corbLinhaAtiva.modo, mensal: displayMensal, reforco: displayReforco })}
        ...
        <div className="no-print" style={{ ...card, borderTop: '3px solid #6b7280' }}>
