...
// (display + itálico) + Public Sans (corpo/nav/controles) — mesmas variáveis
...
export default function EmpreendimentoTemplate({ data }: { data: Empreendimento }) {
  ...
  const CSS = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .ec-btn { display: inline-flex; align-items: center; gap: 8px; padding: 14px 30px; font-family: var(--font-public-sans), system-ui, sans-serif; font-size: 15px; font-weight: 600; text-decoration: none; border: 1px solid ${ACCENT}; color: ${ACCENT}; background: transparent; cursor: pointer; transition: all .25s ease; border-radius: 2px; }
    .ec-btn:hover { background: ${ACCENT}; color: #fff; }
    .ec-btn--solid { background: ${ACCENT}; color: #fff; }
    .ec-btn--solid:hover { background: #9A3717; border-color: #9A3717; }
    .ec-rule { width: 40px; height: 1px; background: ${ACCENT}; border: none; display: block; }
    .ec-h2 { font-family: var(--font-piazzolla), Georgia, serif; font-weight: 600; line-height: 1.2; }
    .ec-serif { font-family: var(--font-piazzolla), Georgia, serif; font-style: italic; font-weight: 400; }
    .ec-diff-item { display: flex; align-items: flex-start; gap: 16px; padding: 18px 0; border-bottom: 1px solid rgba(26,24,20,0.10); }
    .ec-diff-icon { width: 36px; height: 36px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: rgba(193,70,31,0.08); border-radius: 50%; font-size: 16px; }
    ...
    <>
      ...
      <nav className="ec-nav">
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(18px,4vw,48px)', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link href="/" style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 17, textDecoration: 'none', color: '#FFFFFF', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
            Stiven Allan
          </Link>
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            <Link href="/empreendimentos" style={{ fontFamily: BODY, fontSize: 15, fontWeight: 500, color: 'rgba(255,255,255,0.9)', textDecoration: 'none', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>Empreendimentos</Link>
            <a href={WPP} target="_blank" rel="noopener noreferrer" data-wpp="nav" data-wpp-emp={data.slug} data-wpp-nome={data.nome} style={{ fontFamily: BODY, fontSize: 15, fontWeight: 600, color: ACCENT_LIGHT, textDecoration: 'none', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>WhatsApp</a>
          </div>
        </div>
      </nav>
      ...
      <section style={{ position: 'relative', height: '100vh', minHeight: 600, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 clamp(24px,6vw,80px)', paddingTop: 68 }}>
          <p className="ec-fade" style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'rgba(245,238,230,0.85)', marginBottom: 24, textShadow: '0 1px 6px rgba(0,0,0,0.4)' }}>
            {data.eyebrow}
          </p>
          <h1 style={{ fontFamily: DISPLAY, fontWeight: 600, lineHeight: 1.15, fontSize: 'clamp(32px,6vw,64px)', color: '#FFFFFF', textShadow: '0 2px 8px rgba(0,0,0,0.5), 0 2px 32px rgba(0,0,0,0.60)', maxWidth: 820 }} className="ec-fade ec-fade-1">
            {data.heroTitulo}
          </h1>
          ...
          <p className="ec-fade ec-fade-3" style={{ marginTop: 20, fontFamily: BODY, fontSize: 13, color: 'rgba(255,255,255,0.65)', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
      ...
      <section id="conceito" style={{ background: '#FAFAF8', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(48px,6vw,96px)', alignItems: 'center' }}>
          <div>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 20 }}>O empreendimento</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(26px,4vw,42px)', color: '#1A1814', marginBottom: 28 }}>
            ...
            <p className="ec-serif" style={{ fontSize: 'clamp(17px,2vw,21px)', color: '#4A453D', lineHeight: 1.7, marginBottom: 24 }}>
              {data.conceitoTextoDestaque}
            </p>
            <p style={{ fontFamily: BODY, fontSize: 15, color: '#6B655B', lineHeight: 1.8, marginBottom: 32 }}>
              {data.conceitoTexto}
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {data.conceitoChips.map(({ ico, label }) => (
                <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(193,70,31,0.08)', color: ACCENT, borderRadius: 2, padding: '7px 14px', fontFamily: BODY, fontSize: 13 }}>{ico} {label}</span>
              ))}
            </div>
          </div>
          <div style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 4, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: 20, left: 20 }}>
              <p style={{ fontFamily: BODY, fontSize: 12, color: 'rgba(245,238,230,0.80)', marginBottom: 4 }}>Status</p>
              <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 18, color: '#F5EEE6' }}>{data.conceitoStatusLabel}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#1A1410', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT_LIGHT, display: 'block', marginBottom: 16 }}>A torre</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#F5EEE6', marginBottom: 16 }}>Números que encantam</h2>
          ...
          <div className="ec-metrics-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 'clamp(32px,4vw,56px)' }}>
            {data.metricas.map(({ val, label }) => (
              <div key={label} style={{ textAlign: 'center' }}>
                <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 'clamp(38px,5.5vw,58px)', color: 'rgba(245,238,230,0.92)', lineHeight: 1 }}>{val}</p>
                <p style={{ fontFamily: BODY, fontSize: 12, color: 'rgba(245,238,230,0.65)', marginTop: 6 }}>{label}</p>
              </div>
            ))}
          </div>
            ...
            <div style={{ marginTop: 64, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: 24, borderTop: '1px solid rgba(245,238,230,0.10)', paddingTop: 48 }}>
              {data.metricasExtras.map(({ n, label }) => (
                <div key={label} style={{ textAlign: 'center', padding: '24px 16px' }}>
                  <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 'clamp(26px,3.6vw,42px)', color: ACCENT_LIGHT, lineHeight: 1 }}>{n}</p>
                  <p style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', marginTop: 8 }}>{label}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <section id="diferenciais" style={{ background: '#FAFAF8', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 16 }}>Diferenciais</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#1A1814' }}>O padrão que transforma</h2>
          </div>
          <div style={{ maxWidth: 900, margin: '0 auto 48px', display: 'flex', flexDirection: 'column', gap: 40 }}>
            {data.diferenciaisGrupos.map(({ titulo, itens }) => (
              <div key={titulo}>
                <p style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: ACCENT, marginBottom: 8 }}>{titulo}</p>
                <div className="ec-diff-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '0 48px' }}>
                  {itens.map(({ ico, title, desc }) => (
                    <div key={title} className="ec-diff-item">
                      ...
                      <div>
                        <p style={{ fontFamily: BODY, fontWeight: 600, fontSize: 15, color: '#1A1814', marginBottom: 4 }}>{title}</p>
                        <p style={{ fontFamily: BODY, fontSize: 14, color: '#6B655B', lineHeight: 1.6 }}>{desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {data.fichaTecnicaExtra.length > 0 && (
            <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 32, borderTop: '1px solid rgba(26,24,20,0.10)' }}>
              <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', color: ACCENT, marginBottom: 16, textAlign: 'center' }}>Também incluso</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                {data.fichaTecnicaExtra.map(item => (
                  <span key={item} style={{ padding: '7px 14px', background: 'rgba(193,70,31,0.07)', borderRadius: 2, fontFamily: BODY, fontSize: 13, color: '#6B655B' }}>{item}</span>
                ))}
              </div>
            </div>
          )}
      ...
      <section id="galeria" style={{ background: '#FAFAF8', padding: '0 clamp(24px,6vw,80px) clamp(64px,8vw,120px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 16 }}>Perspectivas</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#1A1814', marginBottom: 16 }}>{data.galeriaTitulo}</h2>
      ...
      {data.plantas.length > 0 && (
        <section id="plantas" style={{ background: '#F5EEE6', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: 56 }}>
              <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 16 }}>Plantas</p>
              <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#1A1814', marginBottom: 16 }}>{data.plantasTitulo}</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 48 }}>
              {data.plantasGrupos.map(({ titulo, categoria }) => {
                ...
                return (
                  <div key={categoria}>
                    <p style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: ACCENT, marginBottom: 16 }}>{titulo}</p>
                    <GalleryWithLightbox
                      galeria={itens}
                      prefix="ecp"
      ...
      <section id="lazer" style={{ background: '#1A1410', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(48px,6vw,96px)', alignItems: 'center' }}>
          ...
          <div>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT_LIGHT, display: 'block', marginBottom: 16 }}>Lazer</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#F5EEE6', marginBottom: 16 }}>{data.lazerTitulo}</h2>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {data.lazer.map(item => (
                <span key={item} style={{ padding: '7px 14px', background: 'rgba(193,70,31,0.18)', borderRadius: 2, fontFamily: BODY, fontSize: 13, color: ACCENT_LIGHT }}>{item}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <section style={{ background: '#FAFAF8', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 16 }}>Localização</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#1A1814', marginBottom: 12 }}>{data.localizacaoTitulo}</h2>
            <p style={{ fontFamily: BODY, fontSize: 15, color: '#6B655B', maxWidth: 420, margin: '0 auto' }}>
          ...
          <div style={{ marginTop: 24, display: 'flex', justifyContent: 'center', gap: 48, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={label} style={{ textAlign: 'center' }}>
                <p style={{ fontFamily: BODY, fontSize: 11, color: ACCENT, marginBottom: 4 }}>{label}</p>
                <p style={{ fontFamily: BODY, fontSize: 15, color: '#1A1814' }}>{val}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: '#FAFAF8', padding: 'clamp(64px,8vw,120px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(26,24,20,0.10)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT, display: 'block', marginBottom: 16 }}>Como funciona</p>
            <h2 className="ec-h2" style={{ fontSize: 'clamp(24px,3.5vw,40px)', color: '#1A1814', marginBottom: 16 }}>Financiamento direto</h2>
              ...
              <>
                ...
                <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left' }}>
                  {data.politicaComercial.condicoes.map(({ titulo, texto }) => (
                    <div key={titulo} style={{ background: 'rgba(193,70,31,0.06)', borderRadius: 4, padding: '16px 20px' }}>
                      <p style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: ACCENT, marginBottom: 6 }}>{titulo}</p>
                      <p style={{ fontFamily: BODY, fontSize: 14, color: '#6B655B', lineHeight: 1.6 }}>{texto}</p>
                    </div>
                  ))}
                </div>
                <p style={{ fontFamily: BODY, fontSize: 13, color: '#8a8378', marginTop: 20, maxWidth: 560, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div className="ec-step-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'clamp(32px,4vw,56px)' }}>
            {passos.map(({ n, titulo, desc }) => (
              <div key={n} style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
                <span style={{ fontFamily: DISPLAY, fontSize: 32, fontWeight: 600, color: 'rgba(193,70,31,0.30)', lineHeight: 1, flexShrink: 0, width: 48 }}>{n}</span>
                <div>
                  <p style={{ fontFamily: BODY, fontWeight: 600, fontSize: 15, color: '#1A1814', marginBottom: 10 }}>{titulo}</p>
                  <p style={{ fontFamily: BODY, fontSize: 14, color: '#6B655B', lineHeight: 1.7 }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: '#1A1410', padding: 'clamp(80px,10vw,140px) clamp(24px,6vw,80px)', textAlign: 'center' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <p style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: ACCENT_LIGHT, display: 'block', marginBottom: 24 }}>Próximo passo</p>
          <h2 className="ec-serif" style={{ fontSize: 'clamp(30px,5vw,50px)', color: '#F5EEE6', lineHeight: 1.25, marginBottom: 24 }}>
            {data.ctaFinalTitulo}
          </h2>
          <hr className="ec-rule" style={{ margin: '0 auto 32px' }} />
          <p style={{ fontFamily: BODY, fontSize: 'clamp(15px,1.8vw,18px)', color: 'rgba(245,238,230,0.65)', lineHeight: 1.7, marginBottom: 40 }}>
            {data.ctaFinalTexto}
          </p>
      ...
      <footer style={{ background: '#1A1410', borderTop: '1px solid rgba(245,238,230,0.08)', padding: 'clamp(32px,4vw,48px) clamp(24px,6vw,80px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <p style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 16, color: '#F5EEE6' }}>Stiven Allan</p>
            <p style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', marginTop: 4 }}>CRECI 60.275 · Imóveis no Sul de Santa Catarina</p>
          </div>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
            <Link href="/" style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', textDecoration: 'none' }}>Início</Link>
            <Link href="/empreendimentos" style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', textDecoration: 'none' }}>Empreendimentos</Link>
            <Link href="/politica-de-privacidade" style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', textDecoration: 'none' }}>Privacidade</Link>
            <Link href="/termos-de-uso" style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', textDecoration: 'none' }}>Termos</Link>
            <Link href="/exclusao-de-dados" style={{ fontFamily: BODY, fontSize: 13, color: 'rgba(245,238,230,0.65)', textDecoration: 'none' }}>Exclusão de Dados</Link>
            <a href={WPP} target="_blank" rel="noopener noreferrer" style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: ACCENT_LIGHT, textDecoration: 'none' }}>WhatsApp</a>
          </div>
        </div>
        <div style={{ maxWidth: 1200, margin: '16px auto 0', paddingTop: 16, borderTop: '1px solid rgba(245,238,230,0.05)' }}>
          <p style={{ fontFamily: BODY, fontSize: 12, color: 'rgba(245,238,230,0.35)' }}>
      ...
      <PropertyFAQ items={data.faq} accent={ACCENT} />
